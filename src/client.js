...
@media (max-width: 600px) {
  ...
  /* Compact title cluster so settings sits right after mode name */
  [data-dsh-mobile-frame] .I58I4q_titleCluster {
    flex: 0 1 auto !important;
    max-width: 220px !important;
  }

  /* Remove gap between settings and download */
  [data-dsh-mobile-frame] .I58I4q_headerUtilities {
  ...
  /* Hide the nArs4W toggle button is removed — user wants it visible as sidebar toggle */
}
`;
    ...
    function apply(ctx) {
      ...
      headerSettingsButton.dataset.dshMobileHeaderSettings = "";
      headerActions.append(headerSidebarButton);
      ...
      function syncHeaderActions(layout) {
        ...
        // Whale button goes FIRST in the title row (leftmost).
        ...
        // Settings button sits right after the mode label, inside titleCluster.
        if (target === titleRow) {
          var modeSlot = target.querySelector('[data-slot="conversation.session.header.actions"]');
          var anchor = modeSlot instanceof HTMLElement ? modeSlot.parentElement : null;
          if (anchor instanceof HTMLElement) {
            if (headerSettingsButton.parentElement !== anchor) anchor.append(headerSettingsButton);
          } else if (headerSettingsButton.parentElement !== target) {
            target.appendChild(headerSettingsButton);
          }
        } else if (headerSettingsButton.parentElement !== target) {
          target.appendChild(headerSettingsButton);
        }
        // Make mode name clickable — triggers the model selector in composer.
        var modeSlot = target.querySelector('[data-slot="conversation.session.header.actions"]');
        if (modeSlot instanceof HTMLElement) {
          modeSlot.style.cursor = 'pointer';
          modeSlot.title = '点击切换模型';
          modeSlot.addEventListener('click', function() {
            var modelTrigger = document.querySelector('[data-slot="conversation.input.model"] > div > button');
            if (modelTrigger instanceof HTMLButtonElement) modelTrigger.click();
          });
        }
      }
